...
(function configureEvergage() {
    try {
       Evergage.configure({
           ...
           trackerUrl: "https://northwell.us-4.evergage.com",
           siteConfigVersion: "79"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableCrossDomainIframeCookies":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["emailAddress","profileId","sfmcContactKey"],"lastModified":1758726916960,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":true,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
/*
 * Northwell Health Salesforce Marketing Cloud Interaction Studio Sitemap
 * Version: 2025.10.03
 * Author: Martech Team
 *
 * This sitemap is designed to address inefficiencies in the previous version,
 * reduce 'unmapped page' interactions, and dynamically generate interaction names
 * for improved campaign tracking and retargeting within Salesforce Marketing Cloud Interaction Studio.
 */
...
(function () {
    ...
    const currentPathname = getPathname(window.location.href);

    // Helper to resolve UTM parameters
    ...
    // Function to generate a clean interaction name from a URL path
    const generateInteractionName = (path) => {
        if (!path || path === '/') {
            return 'Homepage';
        }
        // Remove leading/trailing slashes, replace hyphens with spaces, and capitalize each word
        const name = path.replace(/^\/|\/$/g, '')
                           .replace(/-/g, ' ')
                           .split(' ')
                           .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                           .join(' ');
        return `Viewed ${name}`; // Prefix with 'Viewed' for clarity
    };

    SalesforceInteractions.init({
        cookieDomain: "northwell.edu",
    }).then(() => {
        const config = {
            global: {
                contentZones: [
                    ...
                    { name: "use_case_2_smart-grid", selector: ".smart-grid" },
                    { name: "BabyImage", selector: ".masonry-container>div:nth-child(5)>div>div:nth-child(1)>div" },
                    { name: "polioImag", selector: ".masonry-container>div:nth-child(6)>div>div:nth-child(1)>div:nth-child(2)" },
                    { name: "footer", selector: ".footer__wrapper" },
                    { name: "raa_buttons", selector: 'a[href$="request-an-appointment"]' },
                    { name: "call_btn", selector: '.generic-banner__button' },
                    { name: "popup_call_btn", selector: 'evg-btn' },
                    // NEW GLOBAL CONTENT ZONES from user request
                    { name: "global_infobar_top", selector: "body" }, // Placed at the top of the body tag
                    // global_exit_intent_popup is a modal, not tied to a specific selector in the sitemap, handled by campaign logic
                ],
                listeners: [
                    SalesforceInteractions.listener("click", ".menu-holder .mega-menu-toggle", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Header menu open" } });
                    }),
                    SalesforceInteractions.listener("click", ".nav-header .fa-times", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Header menu close" } });
                    }),
                    SalesforceInteractions.listener("click", ".site-logo", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Header Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".fa-facebook", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Footer Facebook Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".fa-twitter", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Footer Twitter Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".fa-instagram", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Footer Instagram Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".fa-linkedin", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Footer Linkedin Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".fa-youtube", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Footer Youtube Logo" } });
                    }),
                    SalesforceInteractions.listener("click", ".generic-banner__button", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Call Button" } });
                    }),
                    SalesforceInteractions.listener("click", ".evg-btn", () => {
                        SalesforceInteractions.sendEvent({ interaction: { name: "Click on : Pop Up Call Button" } });
                    })
                ],
            },
            pageTypeDefault: {
                name: "Default Page",
                interaction: {
                    name: () => generateInteractionName(currentPathname),
                },
            },
            pageTypes: [
                // Homepage
                {
                    name: "Northwell Homepage",
                    isMatch: () => currentPathname === '/' && currentHostname === 'www.northwell.edu',
                    interaction: {
                        name: "Viewed Northwell Homepage",
                    },
                    contentZones: [
                        { name: "test", selector: ".hero__cta" },
                        { name: "homepage_hero", selector: ".hero-blue_box" },
                        { name: "homepage_background", selector: ".hero-video-overlay" },
                        { name: "switch-desc", selector: ".ct-active" },
                        { name: "switch", selector: ".switch-title" },
                        { name: "ourwait", selector: ".section-switch>div:nth-child(5)>p" },
                        { name: "selectdepartment", selector: ".switch-desc" },
                        // NEW HOMEPAGE CONTENT ZONE from user request
                        { name: "homepage_hero_banner", selector: ".hero-xl-cta" } // Updated selector based on observation
                    ],
                },
                // Subdomain Homepages (e.g., jobs.northwell.edu, feinstein.northwell.edu)
                {
                    name: "Subdomain Homepage",
                    isMatch: () => currentPathname === '/' && currentHostname.endsWith('.northwell.edu') && currentHostname !== 'www.northwell.edu',
                    interaction: {
                        name: () => `Viewed ${currentHostname.split('.')[0].replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} Homepage`,
                    },
                },
                // Campaign Landing Pages (generalized)
                {
                    name: "Campaign Landing Page",
                    isMatch: () => {
                        const urlHasUtmParams = utmParams.utm_campaign !== undefined && utmParams.utm_medium !== undefined && utmParams.utm_source !== undefined;
                        // This can be further refined to match specific campaign URL patterns if needed
                        return urlHasUtmParams;
                    },
                    interaction: {
                        name: () => `Viewed Campaign Landing Page: ${utmParams.utm_campaign || 'Unknown Campaign'}`,
                    },
                },
                // Service Line & Condition Pages (e.g., /cancer-care, /heart-care, /conditions/brain-tumor)
                {
                    name: "Service Line or Condition Page",
                    isMatch: () => /\/(cancer-care|heart-care|conditions|services)\/[a-zA-Z0-9-]+/.test(currentPathname),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                    contentZones: [
                        { name: "service_page_sidebar_doctors", selector: ".main-content .sidebar" }, // Append or prepend within sidebar
                        { name: "service_page_bottom_articles", selector: ".main-content" } // Insert after main content, before footer
                    ],
                },
                // Find Care / Find a Doctor pages
                {
                    name: "Find Care Page",
                    isMatch: () => currentPathname.startsWith('/find-care'),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
                {
                    name: "Doctor Detail Page",
                    isMatch: () => currentPathname.startsWith('/find-care/find-a-doctor/') && currentPathname.split('/').filter(Boolean).length > 3,
                    interaction: {
                        name: () => {
                            const doctorNameSegment = currentPathname.split('/').filter(Boolean).pop();
                            return `Viewed Doctor Detail Page: ${doctorNameSegment ? doctorNameSegment.replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : 'Unknown Doctor'}`;
                        },
                    },
                    catalog: {
                        Doctor: {
                            _id: () => {
                                const doctorId = window.location.pathname.split("-").at(-1);
                                return doctorId > 0 ? doctorId : false;
                            },
                            name: () => Evergage.cashDom("h1[class^='BioCard__DocName']").text(),
                            url: Evergage.resolvers.fromHref(),
                            rating: Evergage.cashDom(".ktdeCU").text(),
                            imageUrl: Evergage.resolvers.fromSelectorAttribute(".BioCard__DoctorImage-sc-1vbtc9h-6", "src"),
                            phone: Evergage.cashDom(".fad-profile-header-phone").text(),
                        },
                    },
                    contentZones: [
                        { name: "doctor_profile_cta_banner", selector: "div[class^='BioCard__StyledBioCardHeader']" } // Insert after header, before main bio
                    ],
                },
                // Find a Doctor (Search Results Page)
                {
                    name: "Find a Doctor Search Results Page",
                    isMatch: () => currentPathname === '/find-care/find-a-doctor/',
                    interaction: {
                        name: "Viewed Find a Doctor Search Results Page",
                    },
                    contentZones: [
                        { name: "find_doctor_featured_slot", selector: ".search-results-list" } // Prepend to the list of doctor cards
                    ],
                },
                // Location Pages
                {
                    name: "Location Listing Page",
                    isMatch: () => currentPathname === '/doctors-and-care/locations',
                    interaction: {
                        name: "Viewed Location Listing Page",
                    },
                },
                {
                    name: "Location Detail Page",
                    isMatch: () => currentPathname.startsWith('/doctors-and-care/locations/') && currentPathname.split('/').filter(Boolean).length > 3,
                    interaction: {
                        name: () => {
                            const locationNameSegment = currentPathname.split('/').filter(Boolean).pop();
                            return `Viewed Location Detail Page: ${locationNameSegment ? locationNameSegment.replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : 'Unknown Location'}`;
                        },
                    },
                    catalog: {
                        Location: {
                            _id: SalesforceInteractions.resolvers.fromJsonLd("identifier"),
                            name: SalesforceInteractions.resolvers.fromJsonLd("name"),
                            url: SalesforceInteractions.resolvers.fromJsonLd("url"),
                            mapURL: SalesforceInteractions.resolvers.fromJsonLd("hasMap"),
                            telephone: SalesforceInteractions.resolvers.fromJsonLd("telephone"),
                            address: SalesforceInteractions.resolvers.fromJsonLd("address.streetAddress"),
                            postalCode: SalesforceInteractions.resolvers.fromJsonLd("address.postalCode"),
                            locality: SalesforceInteractions.resolvers.fromJsonLd("address.addressLocality"),
                            category: SalesforceInteractions.resolvers.fromJsonLd("additionalProperty[0].value"),
                        },
                    },
                    contentZones: [
                        { name: "location_page_services_spotlight", selector: ".main-content" } // Insert after main description
                    ],
                },
                // Clinical Trials Pages (generalized)
                {
                    name: "Clinical Trials Page",
                    isMatch: () => currentPathname.startsWith('/clinical-trials'),
                    interaction: {
                        name: () => {
                            const topicParam = utmParams.topic;
                            return topicParam ? `Viewed Clinical Trials: ${topicParam.replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}` : generateInteractionName(currentPathname);
                        },
                    },
                },
                // News & Insights Pages
                {
                    name: "News & Insights Page",
                    isMatch: () => currentPathname.startsWith('/news') || currentPathname.startsWith('/newsroom'),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
                // Manage Your Care Pages
                {
                    name: "Manage Your Care Page",
                    isMatch: () => currentPathname.startsWith('/manage-your-care') || currentPathname.startsWith('/billing-and-insurance'),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
                // About Northwell Pages
                {
                    name: "About Northwell Page",
                    isMatch: () => currentPathname.startsWith('/about-northwell') || currentPathname.startsWith('/about/'),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
                ...
                {
                    name: "Help & FAQ Page",
                    isMatch: () => currentPathname.startsWith('/help'),
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
                // Generic Subdomain Pages (catch-all for subdomains not explicitly handled)
                {
                    name: "Generic Subdomain Page",
                    isMatch: () => currentHostname.endsWith('.northwell.edu') && currentHostname !== 'www.northwell.edu' && currentPathname !== '/',
                    interaction: {
                        name: () => `Viewed ${currentHostname.split('.')[0].replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} ${generateInteractionName(currentPathname)}`,
                    },
                },
                // Default catch-all for any remaining pages on www.northwell.edu
                {
                    name: "General Content Page",
                    isMatch: () => currentHostname === 'www.northwell.edu',
                    interaction: {
                        name: () => generateInteractionName(currentPathname),
                    },
                },
        ...
        // Re-initialize sitemap on URL change to handle SPA navigation
